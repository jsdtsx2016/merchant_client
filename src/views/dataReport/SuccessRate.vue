...
import { getSuccessRate } from "@/api/modules/dataReport";
...
const dataCallback = (data: ResPage<DataReport.SuccessRate>) => {
    const newList: DataReport.SuccessRate[] = [];
    const temp: { [key: string]: DataReport.SuccessRate } = {};
    for (let i = 0; i < data.list.length; i++) {
        const item = data.list[i];
        item["id"] = item.productID + '-' + item.amount;
        if (!temp[item.productID]) {
            ...
            temp[item.productID] = parentData;
            newList.push(parentData);
        }
        else {
            const parentData = temp[item.productID];
            parentData.successOrders += item.successOrders;
            parentData.totalOrders += item.totalOrders;
        }
        item.productName = item.productName + ' ' + (item.amount / 100) + "元";
        temp[item.productID].children.push(item);
        temp[item.productID].children.sort((a, b) => {
...
const getTableList = (params: any) => {
    ...
    return getSuccessRate(newParams);
};
...
// 表格配置项
const columns = reactive<ColumnProps<DataReport.SuccessRate>[]>([
    {
        prop: "updateTime",
        label: "修改时间",
        isShow: false,
    ...
    },
    {
        ...
        label: "产品编号",
        isShow: false,
        search: { el: "input", tooltip: "编号，必须输全，不支持模糊查询" }
    },
    {
        prop: "agentName",
        label: "商户名称",
        isShow: false,
        search: { el: "input", tooltip: "商户的名称，必须输全" },
    },
    {
        prop: "productName",
        label: "产品名称",
        search: { el: "input", tooltip: "产品的名称，支持模糊查询" },
    },
    // {
    //   prop: "group",
    //   label: "分组",
