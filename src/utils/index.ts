...
export function getShowMenuList(menuList: Menu.MenuOptions[]) {
  ...
  return newMenuList.filter((item) => {
    ...
    return !item.meta?.isHide && (userStore.userInfo.roleData.authMenuList.indexOf(item.path) != -1 || item.children?.length);
  });
}
/**
 * @description 使用递归过滤出需要渲染在树形菜单的列表 (需剔除 isHide == true 的菜单)
 * @param {Array} menuList 菜单列表
 * @returns {Array}
 * */
export function getShowTreeList(menuList: Menu.MenuOptions[]) {
  const userStore = useUserStore();
  let newMenuList: Menu.MenuOptions[] = JSON.parse(JSON.stringify(menuList));
  return newMenuList.filter((item) => {
    item.children?.length && (item.children = getShowTreeList(item.children));
    return !item.meta?.isHide && (userStore.userInfo.roleData.authMenuList.indexOf(item.path) != -1 || item.children?.length);
  });
}
