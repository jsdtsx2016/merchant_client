...
<template>
  ...
  <SearchForm v-show="isShowSearch" :search="_search" :reset="_reset" :columns="searchColumns"
    :search-param="searchParam" :show-export="showExport" :search-col="searchCol" :excel="_excel" />
  <!-- 表格主体 -->
  <div class="card table-main">
    ...
    <!-- 表格主体 -->
    <el-table :height="height" ref="tableRef" v-bind="$attrs" :data="processTableData" :border="border"
      :row-key="rowKey" @selection-change="selectionChange" @sort-change="sortChange">
      <!-- 默认插槽 -->
      <slot />
...
const emit = defineEmits<{
  ...
  sortChange: [{ prop: string; sort: -1 | 1 | null }]
}>();
...
const sortChange = (data: { column: any, prop: any, order: any }) => {
  emit("sortChange", { prop: data.prop, sort: sortMap[data.order] });
};
